...
import com.yr.management.common.annotation.Limit;
import com.yr.management.common.authentication.JwtToken;
...
import com.yr.management.common.utils.*;
import com.yr.management.monitor.entity.ActiveUser;
import com.yr.management.monitor.entity.LoginLog;
import com.yr.management.monitor.mapper.LoginLogMapper;
import com.yr.management.monitor.service.ILoginLogService;
...
public class EMLoginController extends BaseController {
    ...
    public LicenseResponse login(
        ...
        if (User.STATUS_LOCK.equals(user.getStatus())) {
            throw new ERPException("账号已被锁定,请联系管理员！");
        }
        ...
        String token = JWTUtil.encryptToken(JWTUtil.sign(username, password, licenseProperties.getShiro().getJwtTimeOut()));
        LocalDateTime expireTime = LocalDateTime.now().plusSeconds(licenseProperties.getShiro().getJwtTimeOut());
    ...
    private Set<ActiveUser> getActiveUser(String username)throws Exception {
        ...
        Set<String> userOnlineStringSet = redisService.zrangeByScore(ERPConstant.ACTIVE_USERS_ZSET_PREFIX, now, "+inf");
    ...
    public void kickout(@NotBlank(message = "{required}") @PathVariable String id) throws Exception {
        ...
        if (kickoutUser != null && StringUtils.isNotBlank(kickoutUserString)) {
            // 删除 zset中的记录
            redisService.zrem(ERPConstant.ACTIVE_USERS_ZSET_PREFIX, kickoutUserString);
            // 删除对应的 token缓存
            redisService.del(ERPConstant.TOKEN_CACHE_PREFIX + kickoutUser.getToken());
        }
    }
    ...
    private String saveTokenToRedis(User user, JwtToken token, HttpServletRequest request) throws Exception {
        ...
        this.redisService.zadd(ERPConstant.ACTIVE_USERS_ZSET_PREFIX, Double.valueOf(token.getExipreAt()), mapper.writeValueAsString(activeUser));
        // redis 中存储这个加密 token，key = 前缀 + 加密 token + .ip
        this.redisService.set(ERPConstant.TOKEN_CACHE_PREFIX + token.getToken(), token.getToken(), licenseProperties.getShiro().getJwtTimeOut() * 1000);
